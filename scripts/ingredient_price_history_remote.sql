-- Paste into Supabase Dashboard → SQL Editor if `ingredient_price_history` is missing remotely.
-- Mirrors: supabase/migrations/20260513231000_ingredient_price_history.sql
...
create index if not exists idx_ingredient_price_history_ingredient_id
  on public.ingredient_price_history(ingredient_id);

create index if not exists idx_ingredient_price_history_ingredient_created
...
drop policy if exists "ingredient_price_history_select_own" on public.ingredient_price_history;
...
drop policy if exists "ingredient_price_history_insert_own" on public.ingredient_price_history;
