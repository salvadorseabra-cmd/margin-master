/**
 * Re-extract all 6 VL invoices after Pass C prompt redesign.
 */
...
const OUT_DIR = ".tmp/passc-implementation/reextract";
...
for (const { id, label } of INVOICES) {
  ...
  summary.push({
    ...
    items: items.map((i: { name?: string; quantity?: number | null }) => ({
      name: i.name,
      quantity: i.quantity,
    })),
  });
