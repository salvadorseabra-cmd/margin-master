/**
 * Final Validation Lab re-run — v26 (April harness fix + Chocolate price isolation).
 */
...
const OUT_DIR = ".tmp/final-validation-lab-rerun-v26";
...
async function imageDataUrlFor(inv: (typeof INVOICES)[number]): Promise<string> {
  if (inv.imageOverride) {
    if (inv.imageOverride.endsWith(".b64.txt")) {
      return normalizeDataUrl(inv.imageOverride);
    }
...
const v25Rerun = fileExists(".tmp/final-validation-lab-rerun/metrics.json")
  ? loadJson<{ aggregate: Record<string, unknown>; perInvoice: Array<Record<string, unknown>> }>(
      ".tmp/final-validation-lab-rerun/metrics.json",
    )
  : null;
...
for (const inv of INVOICES) {
  console.log(`[v26-rerun] ${inv.label}`);
...
const globalMetrics = {
  ...
  deployVersion,
  harnessFixes: [
    "Aviludo April: b64 data URL prefix normalization (no double prefix)",
    "Chocolate Pantagruel: v26 row-isolation monetary prompt",
  ],
  ...
  baselineComparison: {
    ...
    v25Rerun: v25Rerun
      ? {
          fieldAccuracy: v25Rerun.aggregate.fieldAccuracy,
          quantityAccuracy: v25Rerun.aggregate.quantityAccuracy,
          financialErrorEuro: v25Rerun.aggregate.financialErrorEuro,
        }
      : null,
    now_v26: {
      fieldAccuracy: avg(perInvoice.map((p) => p.fieldAccuracy as number)),
  ...
  JSON.stringify(
    {
      ...
      deltaVsV25: v25Rerun
        ? {
            fieldAccuracy: round2(
              (globalMetrics.aggregate.fieldAccuracy as number) -
                (v25Rerun.aggregate.fieldAccuracy as number),
            ),
            quantityAccuracy: round2(
              (globalMetrics.aggregate.quantityAccuracy as number) -
                (v25Rerun.aggregate.quantityAccuracy as number),
            ),
            financialErrorEuro: round2(
              (globalMetrics.aggregate.financialErrorEuro as number) -
                (v25Rerun.aggregate.financialErrorEuro as number),
            ),
          }
        : null,
      notes: [
        "April harness: normalize b64 data URL (fixes false 0-item regression)",
        "Chocolate v26: row-isolation monetary prompt deployed",
        "Emporio discount rows: GT gross vs visible net",
        "Pomodor GT qty mismatch vs visible invoice",
      ],
    },
    null,
    2,
