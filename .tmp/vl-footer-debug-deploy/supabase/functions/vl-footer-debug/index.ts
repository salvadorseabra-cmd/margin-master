...
import { callOpenAiJson } from "../extract-invoice/invoice-date-extraction.ts";
import { cropBottomPortion } from "../extract-invoice/invoice-image-crop.ts";
import { parseFooterMetadataExtraction } from "../extract-invoice/invoice-footer-metadata-parse.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({ rawGptJson, parsed }), {
      headers: { ...cors, "Content-Type": "application/json" },
    });
  } catch (e) {
    return new Response(
