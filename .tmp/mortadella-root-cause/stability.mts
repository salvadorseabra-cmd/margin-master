/**
 * Mortadella v28 stability — 3 Emporio invokes (read-only)
 */
...
const RUNS = 3;
const GT = { qty: 3.11, unit_price: 10.1, total: 31.07 };
...
for (let run = 1; run <= RUNS; run++) {
  ...
  const row = items.find((i) => /mortadella/i.test(i.name)) ?? null;
  const finErr = row ? Math.abs(row.total - GT.total) : GT.total;
  const qtyUnitSynth = row ? Math.abs(row.quantity * row.unit_price - row.total) < 0.15 : false;
  runs.push({
    run,
    status: res.status,
    ...
    appearsQtyTimesUnit: qtyUnitSynth,
  });
  console.log(
    `run ${run} qty=${row?.quantity} unit=${row?.unit_price} total=${row?.total} €err=${finErr.toFixed(2)} synth=${qtyUnitSynth}`,
  );
  ...
  JSON.stringify(
    {
      ...
      groundTruth: GT,
      visible: { qty: 3.11, gross_unit: 11.1, discount_pct: 10, total: 31.07 },
      runs,
      summary: {
        perfectTotalRuns: `${perfect}/${RUNS}`,
        stabilityPercent: Math.round((perfect / RUNS) * 100),
        totalUnique: [...new Set(runs.map((r) => (r.mortadella as { total?: number })?.total))],
