/** Invoke extract-invoice on table-data-crop — closer OCR proxy */
...
const png = readFileSync(`${OUT}/table-data-crop.png`);
...
writeFileSync(
  `${OUT}/table-crop-extract-response.json`,
...
console.log(JSON.stringify({ status: res.status, ginger, allNames: (body?.items ?? []).map((i: { name: string }) => i.name) }, null, 2));
