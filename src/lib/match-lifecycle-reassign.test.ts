...
import { reassignMatch } from "@/lib/match-lifecycle-service";
...
const PEPINO_FRESCO_ID = "f1f0e0d0-c000-4000-8000-000000000001";
...
const MOZZ_A_ID = "moz-a-1111-1111-1111-111111111111";
const MOZZ_B_ID = "moz-b-2222-2222-2222-222222222222";
const MOZZ_INVOICE_ID = "inv-mozzarella-001";
const MOZZ_LINE_ID = "line-mozzarella-001";
const MOZZ_A_HISTORY_ID = "hist-moz-a-invoice";
...
function createHistoryPricingMock(options: {
  matchRows: Map<string, InvoiceItemMatchRow>;
  historyRows: Map<string, HistoryRow>;
  ingredients: Map<
    string,
    {
      id: string;
  ...
  const { matchRows, historyRows, ingredients } = options;
                      ...
                      limit() {
                        return {
                          maybeSingle: async () => {
                            ...
                            return top
                              ? {
                                  data: { new_price: top.new_price, invoice_id: top.invoice_id },
                                  error: null,
                                }
                    ...
                    return {
                      select: async () => {
                        const toDelete = [...historyRows.entries()].filter(
                          ([, row]) =>
                            row.ingredient_id === value &&
                            (col2 === "invoice_id" ? row.invoice_id == val2 : false),
                        );
                        for (const [id] of toDelete) historyRows.delete(id);
                        return { data: toDelete.map(([id]) => ({ id })), error: null };
                      },
...
function createPepinoReassignFixture() {
  ...
  const client = createHistoryPricingMock({ matchRows, historyRows, ingredients });
  return { client, matchRows, historyRows, ingredients };
}

function createMozzarellaReassignFixture() {
  const matchRows = new Map<string, InvoiceItemMatchRow>([
    [
      MOZZ_LINE_ID,
      {
        invoice_item_id: MOZZ_LINE_ID,
        user_id: "user-1",
        invoice_id: MOZZ_INVOICE_ID,
        ingredient_id: MOZZ_A_ID,
        status: "confirmed",
        match_kind: "exact",
  ...
  const historyRows = new Map<string, HistoryRow>([
    [
      MOZZ_A_HISTORY_ID,
      {
        id: MOZZ_A_HISTORY_ID,
        ingredient_id: MOZZ_A_ID,
        invoice_id: MOZZ_INVOICE_ID,
        ingredient_name: "Mozzarella A",
        ...
        delta_percent: null,
        created_at: "2026-06-01T12:00:00.000Z",
        invoices: { invoice_date: "2026-06-01", created_at: "2026-06-01T12:00:00.000Z" },
      },
    ],
  ]);
...
describe("reassign subtractive cleanup — Pepino conserva → fresco", () => {
  beforeEach(() => {
  ...
  it("deletes conserva poison history before reassignMatch to fresco", async () => {
    const { client, matchRows, historyRows } = createPepinoReassignFixture();

    correctionMemory.rejectIngredientMatchPair({
      itemName: "Pepino",
      rawItemName: "Pepino",
    ...
    expect(correctionMemory.rejectIngredientMatchPair).toHaveBeenCalledWith(
      expect.objectContaining({ rejectedIngredientId: PEPINO_CONSERVA_ID }),
    );

    const dualAttributionOnA = [...historyRows.values()].some(
      (row) => row.invoice_id === BIDFOOD_INVOICE_ID && row.ingredient_id === PEPINO_CONSERVA_ID,
    );
    expect(dualAttributionOnA).toBe(false);
  });
});

describe("reassign subtractive cleanup — Mozzarella A → B", () => {
  beforeEach(() => {
    vi.spyOn(matchLifecycleFlags, "isMatchLifecycleReassignSubtractiveEnabled").mockReturnValue(true);
    vi.spyOn(matchLifecycleFlags, "isMatchLifecycleSubtractivePricingEnabled").mockReturnValue(true);
  });
  ...
  it("removes invoice history from A so no dual attribution with B forward write", async () => {
    const { client, historyRows } = createMozzarellaReassignFixture();

    const cleanup = await subtractivePricingCleanupForReassign(client, {
      invoiceId: MOZZ_INVOICE_ID,
      previousIngredientId: MOZZ_A_ID,
      wasConfirmed: true,
    });

    expect(cleanup.historyDeleted).toBe(true);
    expect(historyRows.has(MOZZ_A_HISTORY_ID)).toBe(false);

    historyRows.set("hist-moz-b-invoice", {
      id: "hist-moz-b-invoice",
      ingredient_id: MOZZ_B_ID,
      invoice_id: MOZZ_INVOICE_ID,
      ingredient_name: "Mozzarella B",
      supplier_name: "Supplier",
      ingredient_unit: "kg",
      previous_price: null,
      new_price: 12.5,
    ...
    const rowsForInvoice = [...historyRows.values()].filter(
      (row) => row.invoice_id === MOZZ_INVOICE_ID,
    );
    expect(rowsForInvoice).toHaveLength(1);
    expect(rowsForInvoice[0]?.ingredient_id).toBe(MOZZ_B_ID);
  });
});

describe("regression — unmatch A→No Match", () => {
  beforeEach(() => {
    vi.spyOn(matchLifecycleFlags, "isMatchLifecycleSubtractivePricingEnabled").mockReturnValue(true);
    vi.spyOn(correctionMemory, "rejectIngredientMatchPair").mockImplementation(() => {});
  });
  ...
  it("still tombstones and deletes history on confirmed unmatch", async () => {
    const { client, matchRows, historyRows } = createPepinoReassignFixture();
    ...
    expect(historyRows.has(POISON_HISTORY_ID)).toBe(false);
    expect(matchRows.get(PEPINO_LINE_ID)?.status).toBe("unmatched");
  });
});
